...
class LoadStageData(TestCase):

    def setUp(self):
        # default values
        self.index_name = 'stage'
    ...
    def test_check_make_docs(self):
        file_path = os.path.join(os.path.dirname(__file__), 'files', '2021-07-26.etapas')

        stage_uploader = StageFile(file_path)
        list(stage_uploader.make_docs())
    ...
    def test_load_profile_data(self, elasticsearch_mock, search_mock, parallel_bulk):
        file_name_list = ['2021-07-26.etapas', '2021-07-26.etapas.gz', '2021-07-26.etapas.zip']
        for file_name in file_name_list:
            ...
            stage_uploader = StageFile(file_path)
            stage_uploader.load(elasticsearch_mock, self.index_name, self.chunk_size, self.threads, self.timeout)

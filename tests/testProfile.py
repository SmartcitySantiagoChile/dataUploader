...
class LoadProfileData(TestCase):
    ...
    def test_check_make_docs(self):
        file_path = os.path.join(
            os.path.dirname(__file__), "files", "2020-11-30-with-evasion-2.profile"
        )
    ...
    def test_load_profile_data(self, elasticsearch_mock, search_mock, parallel_bulk):
        file_name_list = [
            "2020-11-30-with-evasion-2.profile",
            "2020-11-30-with-evasion-2.profile.gz",
            "2020-11-30-with-evasion-2.profile.zip",
            "2017-07-31-with-evasion.profile",
            "2017-07-31-with-evasion.profile.gz",
            "2017-07-31-with-evasion.profile.zip",
            "2020-03-20.profile",
            "2020-03-20.profile.gz",
            "2020-03-20.profile.zip",
            "2021-06-30.profile",
            "2021-06-30.profile.gz",
            "2021-06-30.profile.zip",
        ]
        ...
        for file_name in current_file_name_list + old_file_name_list:
            ...
            with data_file.get_file_object() as csvfile:
                ...
                if file_name in current_file_name_list:
                    ...
                    self.assertTrue(
                        all(col in profile_uploader.fieldnames for col in row)
                    )
